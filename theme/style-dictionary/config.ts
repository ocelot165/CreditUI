  ...
  if (obj.hasOwnProperty('value')) {
    ...
    for (let name in obj) {
      if (obj.hasOwnProperty(name)) {
        toRet[lowercase ? name.toLocaleLowerCase() : name] = minifyDictionary(
          obj[name],
          {
            lowercaseKey,
            levels: levels - 1,
...
StyleDictionary.registerFormat({
  ...
  formatter: function ({ dictionary }) {
    ...
    return JSON.stringify(
      minifyDictionary(dictionary.tokens, { lowercaseKey: true, levels: 3 }),
      null,
      2,
    );
  },
...
StyleDictionary.registerFormat({
  name: 'json/shadow',
  formatter: function ({ dictionary }) {
    const elevation = Object.keys(dictionary.tokens.Elevation).reduce(
      (memo, key) => {
        const { value } = dictionary.tokens.Elevation[key];
        if (Array.isArray(value)) {
          memo[key.toLowerCase().replace(/\s/g, '')] =
            getDropShadow(value[0]) + ' ' + getDropShadow(value[1]);
        } else {
      ...
      files: [
        {
          ...
          filter: function (token) {
            return ['Primary', 'Secondary', 'Alert', 'Success'].includes(
